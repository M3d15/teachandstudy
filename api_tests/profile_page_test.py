...
@allure.title('Retrieve users data')
def test_user_me(base_api_url, access_token, credentials):
    ...
    with allure.step('Verify the username with one in the response'):
        assert credentials.get('username') == json_user_me_data.get('username')


@allure.story('Profile page')
@allure.title('Change users data')
def test_change_users_data(base_api_url, access_token):
    path = "/api/v1/profile/"
    ...
    with allure.step('Verify users data with one in the response'):
        with allure.step('Verify first_name with one in the response'):
        ...
        with allure.step('Verify patronymic with one in the response'):
            assert json_random_data.get('patronymic') == response_changed_data.get('patronymic')
        with allure.step('Verify phone with one in the response'):
            assert json_random_data.get('phone') == response_changed_data.get('phone')


@allure.story('Profile page')
@allure.title('Change users password')
